...
- `src/lib/crypto/secrets.ts` — **at-rest envelope encryption (AES-256-GCM) for provider secrets.** `encryptSecret`/`decryptSecret` wrap the 4 secret columns at the `rowToTenant`/`tenantToRow` boundary (`slack_webhook_url`, `google_search_console_key`, `instagram_access_token`, `revalidation_secret`) and the `accessToken`/`refreshToken`/`apiKey` fields in `connections.ts`. **INERT until `SECRETS_ENC_KEY` is set** — with no key, writes stay plaintext, so deploying the code changes nothing at rest. Reads are dual-mode: a legacy plaintext value (no `enc:v1:` prefix) passes through, an enveloped value is decrypted, so a mixed table reads correctly at any point in the rollout. Activation = set `SECRETS_ENC_KEY` (any passphrase; sha256'd to the key) in Vercel → run `scripts/backfill-secret-encryption.ts` (idempotent, refuses without the key). Every write of those secrets routes through `upsertTenant`/`tenantToRow` or `saveConnection`, so activation fully covers them; do NOT write those columns/Redis connection blobs by any other path. Residual: the `reb:tenants:all` Redis list cache holds a decrypted copy (60s TTL) — correct for reads but outside the at-rest boundary; optional defense-in-depth.
