...
- **Supabase Postgres is the source of truth for tenant + content + operational data in production** (flipped 2026-06-20). Reads/writes go through `src/lib/db/repositories.ts` + the per-store path, gated by `CONTENT_SOURCE`, `TENANTS_SOURCE`, `DATA_SOURCE` (= `postgres` in prod); the non-Postgres arm is the local dev-file (or Redis) fallback. **No store reads or writes Sanity anymore** — the shadow-writes and fallback reads were all removed in the 2026-07-10 teardown. Rollback is therefore forward-only (restore from a Postgres backup), not a flag-flip back to Sanity; this is acceptable because Sanity is decommissioned. **Tenant isolation is enforced at the APPLICATION layer** — every API route derives the tenant from trusted auth/headers and calls `requireTenantAccess`; the control plane talks to Postgres via a service-role client that BYPASSES row-level security. RLS policies exist (`supabase/migrations/*_rls.sql`) and are kept current as future defense-in-depth, but they are NOT the live enforcement boundary today. The dev-file path remains the local-dev fallback.
...
- `src/lib/auth.ts` — Supabase Auth (`auth.uid()`) + per-tenant roles via the `memberships` table + super-admin via `super_admins`. Supabase-only as of #146 (the Clerk dual-path is gone); `src/proxy.ts` owns the request-level auth gate + `isPublicRoute`/`isCronRoute` matcher (hand-rolled, fail-closed)
