...
- **Governed-work Postgres mirror (Phase 2 ontology):** the governed-work lifecycle ALSO shadows into normalized Postgres tables `proposals` / `decisions` / `execution_attempts` / `outcomes` (`src/lib/governed-work/`, migration `20260714210000`). Two flags in `src/lib/db/dual-write.ts`: **`GOVERNED_WORK_DUAL_WRITE` — LIVE in prod** (best-effort shadow-writes on `addEvent`/resolve/execute via `shadow.ts`; flag-off = zero writes), and **`GOVERNED_WORK_READ_PG` — OFF** (when on, `getEvents`/`getEvent` hydrate governed events from Postgres via the `proposalToEvent` reverse-mapper in `read.ts`; flag-off = byte-identical Redis path). **Redis stays authoritative** — the tables are a verified shadow, not the source of truth. Parity is VERIFIED (`scripts/governed-work-parity.ts` → 97/97; backfill of current state: `scripts/backfill-governed-work.ts`). ⚠️ Before the authority flip: custom `change_request` workflow states move through `updateEvent` (not `resolveEvent`), so they leave no decision row and would reconstruct as `pending` — that gap must be closed (shadow `updateEvent`) first, and membership/status-filtering is deliberately kept Redis-authoritative so `getOpenChangeRequest`/`getQueueCount` can't be corrupted meanwhile. Only `proposals` is tenant-scoped (children FK-cascade); it's in the `deprovision-tenant.ts` sweep.
