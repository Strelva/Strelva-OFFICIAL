...
- **Supabase Postgres is the source of truth for tenant + content + operational data in production** (flipped 2026-06-20). Reads/writes go through `src/lib/db/repositories.ts` + the per-store path, gated by `CONTENT_SOURCE`, `TENANTS_SOURCE`, `DATA_SOURCE` (= `postgres` in prod); the non-Postgres arm is the local dev-file (or Redis) fallback. **No store reads or writes Sanity anymore** — the shadow-writes and fallback reads were all removed in the 2026-07-10 teardown. Rollback is therefore forward-only (restore from a Postgres backup), not a flag-flip back to Sanity; this is acceptable because Sanity is decommissioned. **Tenant isolation is enforced at the APPLICATION layer** — every API route derives the tenant from trusted auth/headers and gates it with `requireTenantAccess` (membership) OR `requireTenantPermission`/`requireTenantPermissions` (`src/lib/auth.ts`). The permission gates already enforce membership (`getTenantRole` → 403 when the caller isn't a member), so a route that has a permission check does NOT also need a redundant `requireTenantAccess` — the 2026-07-13 hardening removed those duplicates. Only a route with NO permission gate (e.g. an authenticated read) keeps a standalone `requireTenantAccess` (media GET is the canonical example). The control plane talks to Postgres via a service-role client that BYPASSES row-level security. RLS policies exist (`supabase/migrations/*_rls.sql`) and are kept current as future defense-in-depth, but they are NOT the live enforcement boundary today. The 2026-07-13 `harden_database_security_boundaries` migration is additive groundwork on the auth-facing objects (RLS on `super_admin_bootstrap`; hardened `security definer` helpers `app_is_super_admin()`/`app_tenant_ids()` and `handle_new_user` with empty `search_path`; FK indexes; a non-negative `site_metrics` check) — still groundwork, not active enforcement. The dev-file path remains the local-dev fallback.
...
> **Migration FLIPPED + live (2026-06-20); Sanity teardown DONE (2026-07-10):** the data + auth backbone is on Supabase Postgres in prod — auth (Supabase), content (`CONTENT_SOURCE=postgres`), tenants (`TENANTS_SOURCE=postgres`), and the operational stores (`DATA_SOURCE=postgres`) all read/write Postgres, verified live. See `docs/supabase-migration-plan.md` + `docs/post-cutover-runbook.md`. The **Sanity data-source teardown is complete** — every store's shadow-write and fallback read is gone, `core.ts` no longer touches Sanity, and the `@sanity/client`/`next-sanity`/`sanity` deps are removed. (`blog.ts` is GONE as of 2026-06-22 — the blog read AND write now go through the Collections CMS / Postgres via `src/lib/cms/blog-public.ts` + the agent's collections-backed blog tools.) **Rollback is now forward-only** (restore from a Postgres backup) for every store — none shadow-write Sanity, so a flag-flip back is no longer a rollback path. **Remaining Sanity work is OPS-only:** rewrite legacy content-image URLs off the Sanity CDN, then lock the dataset + unset `SANITY_*` env, then delete the residual `sanityImageUrl` resolver (+ `@sanity/image-url` + `cdn.sanity.io` CSP). **Clerk teardown is DONE (2026-07-11, #146)** — `@clerk` dep dropped, `auth.ts` Supabase-only, `proxy.ts` hand-rolled auth gate + route matcher; no Clerk imports remain.
