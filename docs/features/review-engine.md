...
## Build notes
- **Done-for-you replies (shipped):** the client is not shown a "draft it yourself" to-do — replies arrive drafted. `poll-google-reviews` drafts new reviews into the queue, and the `review-auto-post` cron drafts the backlog. Behaviour is driven by the per-tenant **`ReplyVoice` mode** (`src/lib/reviews/reply-voice.ts`): `off` (don't touch reviews) / `approve` (draft → owner taps approve) / `auto` (draft → auto-posts after a **12h safety window** unless edited/cancelled). Auto-post publishes through the SAME governed `resolveEventAction("approved")` path — no new external-write path. This is the ONE deliberate relaxation of "never auto-publish external without approval": per-client `auto` opt-in only, and only after the window. The reply drafter mirrors the client's voice guidance + example templates. See AGENTS.md "Done-for-you review replies" + `src/lib/reviews/auto-reply.ts`.
