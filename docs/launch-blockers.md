...
- `pnpm test` passes: 259 tests across 28 files.
...
- `REB_DEV_UNGATED_ACCESS=0 pnpm smoke` passes: 19 Playwright tests on an isolated local port so an unrelated `localhost:3000` server cannot satisfy smoke checks. The suite covers direct `/dashboard` access, signed-out `/account` handoff, signed-out `/no-access` recovery, rendered auth page titles, root and admin-host sign-up guidance, the `admin.gldf.localhost` dashboard entry point, and cron protection.
- `pnpm check:launch` passes end to end with lint, typecheck, 259 unit tests across 28 files, `pnpm audit`, `pnpm build`, and 19 smoke tests with `REB_DEV_UNGATED_ACCESS=0`.
...
- The `/no-access` recovery page now signs users out before returning them to `/sign-in`, so a customer who used the wrong account can switch to the invited email without getting stuck in the same Clerk session.
- `docs/production-readiness.md` now includes a Customer Access Handoff for `/admin` invites, `/api/admin/invites`, exact invited-email sign-up, wrong-account recovery, and final authorized dashboard verification.
...
- Integration OAuth callbacks now verify signed, expiring state before saving tenant connections.
