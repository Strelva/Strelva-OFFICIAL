...
- Content section, version restore, and page-config write routes now reject malformed or non-object JSON with `400 Invalid request body` before validation/storage work.
- Dashboard queue approvals, event actions, and inbox mark-read writes now reject malformed or non-object JSON with `400 Invalid request body` before event or inbox storage work.
