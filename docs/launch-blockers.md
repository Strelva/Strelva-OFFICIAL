...
- Dashboard queue approvals, event actions, inbox mark-read, thread create/update writes, chat transcript saves, suggestion actions, and newsletter sends now reject malformed or invalid-shape JSON with `400 Invalid request body` before event, inbox, thread, chat, suggestion, or newsletter storage/send work.
