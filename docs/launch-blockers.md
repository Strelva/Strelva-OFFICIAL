...
- Dashboard queue approvals, event actions, inbox mark-read, thread create/update writes, chat transcript saves, suggestion actions, newsletter sends, and review add/reply writes now reject malformed or invalid-shape JSON with `400 Invalid request body` before event, inbox, thread, chat, suggestion, newsletter, or review storage/send work.
