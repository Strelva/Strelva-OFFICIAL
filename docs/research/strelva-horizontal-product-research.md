...
```mermaid
flowchart TD
  Entry[Bring a problem, file, link, or existing process] --> Work[Work: outcome, scope, people, progress]
  Work --> Change[Proposed change and review]
  Change --> Existing[Change an existing tool]
  Change --> Result[Produce a document or other result]
  Change --> App[Create or update an application]
  Existing --> Proof[Receipt and completion evidence]
  Result --> Proof
  App --> Proof
  Proof --> Responsibility[Keep doing it within agreed limits]
  Responsibility --> Work
  People[Customer, Strelva, agency, specialist agent] --> Work
```
