...
These are open audit findings that affect this system. Fix them before enabling any production feature that touches the relevant path.

**[HIGH][SSRF] AI-visibility scorer fetches user-supplied URLs without pre-validation (`src/lib/ai-visibility/score.ts:80-95`).** `scoreAiVisibility` calls `fetchText` on the caller-supplied URL before running any SSRF check — there is no `validateUrlSafety` call at the top of the function, unlike `runAudit` which calls it first. Also, `fetchText` uses `redirect: "follow"` with no post-redirect DNS re-check, so a public-to-private redirect (DNS rebinding) delivers the response from an internal host after the initial check. Fix: import and call `validateUrlSafety(url)` at the top of `scoreAiVisibility` before the `fetchText` calls. Add a post-redirect host re-validation inside `fetchText` itself, matching the `buildAuditContext` pattern in `checks.ts`. This is separate from the main `runAudit` engine (which is already protected) and affects the CLI scorecard and any surface that calls `scoreAiVisibility`.

**[HIGH][bug] Missing `Cache-Control: private` on collections list and single-entry v1 routes (`src/app/api/v1/collections/[tenant]/[type]/route.ts:39`, `src/app/api/v1/collections/[tenant]/[type]/[slug]/route.ts:38`).** Both routes return `NextResponse.json(...)` without a `Cache-Control` header, leaving the response subject to CDN-level public caching — a different tenant's entries could be served to an unrelated request. The content, page-config, and site-capabilities routes already use `Cache-Control: private, max-age=0, must-revalidate`. Define a `TENANT_PRIVATE_CACHE` constant and add it as a header to every successful JSON response in both collections routes.

**[HIGH][bug] Tenant rename registry missing several Redis-authoritative keys (`src/lib/tenant-rename.ts:32`).** `authoritativePatterns` does not include `google-meta:${t}` (GBP write state — silently stranded on rename), `review-replies:recent:${t}`, `reb:review-nudge-sent:${t}`, `reb:order-review-request-sent:${t}:*`, or `reb:review-reply-declined:${t}:*` (the declined-reply veto is 180-day durable). A tenant rename silently orphans these keys under the old slug. Add all five patterns to `authoritativePatterns` and update the completeness unit test assertions to cover them.

**[MEDIUM][security] Newsletter HTML sanitizer allows CSS expressions and `javascript:` URLs in style attributes (`src/lib/email-html.ts:23-33`).** `sanitizeEmailHtml` allows the `style` attribute in `ALLOWED_ATTR`, which DOMPurify does not strip for CSS `url()` or `expression()` patterns. Simplest fix: remove `'style'` from `ALLOWED_ATTR` — tenant-supplied newsletter body content should not need raw CSS.

**[MEDIUM][security] Rate-limit on `/api/audit/scan` opens to unlimited throughput when Redis is absent (`src/app/api/audit/scan/route.ts:16-19`).** `checkRateLimit` returns `{ allowed: true }` when `getRedis()` returns null, so a Redis outage (or a dev env with no Redis configured) removes the public rate limit entirely. Consider fail-closed: return `{ allowed: false }` when Redis is unavailable, or at minimum log/alert on the bypass.
