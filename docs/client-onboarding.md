...
**Owners:** **J** = Jacob (GTM + builds the custom repo) · **N** = Noah (platform / dev / manage) · **Auto** = the provision script · **Client** = the client (with our guidance).
...
## Phase 1 — Provision the tenant (Auto, run by N)
...
## Phase 2 — Build the custom repo (J)
...
- [ ] **Build-QA audit — `pnpm audit:full <the-vercel-build-url>`. Target grade B+ before launch.** The recurring gaps to close on every build (see the portfolio audit): an **FAQ section**, a working **`llms.txt`**, `sameAs` social profiles in the Org schema, a Team/About link, trust/credentials, security headers, labeled form fields. Fix these in `custom-repo-starter` once so future builds inherit them.

**Verify:** the build's audit grade is B or better and the site renders real content server-side (not a JS shell).

---

## Phase 3 — Activation (J for beacons, Client for grants)
...
- [ ] **GSC:** client adds the `strelva-reporting` service account to their Search Console property (Settings → Users and permissions).
...
## Phase 4 — Domain & DNS (Client sets records, N attaches)
...
## Phase 5 — Set the launch fields on the tenant (N)
...
## Phase 8 — Hand the client the keys (N/J)
