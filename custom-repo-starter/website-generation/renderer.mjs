...
function renderContact(content) {
    ...
    const links = [];
    if (contact.email)
        links.push(`<a href="${escapeHtml(safeHref(`mailto:${stringValue(contact.email)}`))}">${escapeHtml(contact.email)}</a>`);
    if (contact.phone)
        links.push(`<a href="${escapeHtml(safeHref(`tel:${stringValue(contact.phone)}`))}">${escapeHtml(contact.phone)}</a>`);
...
function renderCta(content, props) {
    const settings = dict(content.settings);
...
function renderPublishedCapabilities(site) {
...
function renderSection(section, content) {
    ...
    switch (section.type) {
        ...
        case "trust-strip": {
            const settings = dict(content.settings);
            const labels = [settings.trustBadge, settings.ownerTitle].filter((value) => typeof value === "string" && Boolean(value.trim()));
...
export function renderGeneratedPage(site, pageKey) {
...
export function renderGeneratedSite(site) {
    return renderGeneratedPage(site, "home");
}
export function renderGeneratedPages(site) {
