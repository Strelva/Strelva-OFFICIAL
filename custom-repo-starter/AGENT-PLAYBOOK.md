...
7. **Native booking, when explicitly published.** If the control plane has published a native booking capability and bound this tenant to a connected schedule, copy **`StrelvaBookingForm.tsx`** and **`booking-client.ts`** together. Mount `StrelvaConnectedBookingForm` with the tenant, approved capability id, and a bounded ISO date range. The component shows only published slots and a safe receipt, then uses the same receipt for change and cancellation. It never receives workspace, work, or provider event identifiers. Do not replace an existing external `ScaffoldBooking` iframe unless the control plane has published this binding and the release gate is open.
   If the page offers both an inquiry and a native booking, copy **`StrelvaInquiryBookingForm.tsx`** as the composition and pass each published capability explicitly. The booking submission records the visitor inquiry before the governed calendar path runs.
