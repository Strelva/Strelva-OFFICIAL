-- Date fields are calendar values, not instants. They stay as YYYY-MM-DD
-- strings through candidate editing, release publication, recipient use and
-- rollback so a browser timezone cannot move a business date.

create or replace function public.application_is_valid_date_only(p_value text)
returns boolean
...
begin
  ...
  year_value := substring(p_value from 1 for 4)::integer;
  if year_value < 1 or year_value > 9999 then return false; end if;
  begin
    parsed := p_value::date;
