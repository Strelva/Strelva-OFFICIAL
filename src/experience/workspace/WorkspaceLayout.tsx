...
import { ArrowLeft, ArrowRight, ArrowUpRight, Building2, FileSearch, Globe2, MessageSquareText, Search, Table2 } from "lucide-react";
...
interface Props {
  ...
  onNew: () => void;
  onAgency: () => void;
  onInquiry?: (tenantId: string) => void;
  onTracker?: () => void;
...
function initialSection(): StrelvaSection {
  ...
  if (value === "tracker" || value === "inquiries") return "work";
  return value === "products" || value === "work" || value === "help" ? value : "home";
}

export function WorkspaceLayout({ appBase, signOut, snapshot, managedWork = [], managedWorkUnavailable, home, agency, busy, selectedWork, onHome, onNew, onAgency, onInquiry, onTracker, inquiryBusinesses = [], inquiry, tracker, onChoose, onWorkspace, notice, children }: Props) {
  ...
  useEffect(() => {
    const restore = () => { setSection(initialSection()); setProductId(null); setQuery(""); setHelpRequest(undefined); };
  ...
  function navigate(next: StrelvaSection, prefillHelp?: string) {
    setSection(next); setProductId(null); setQuery(""); setHelpRequest(next === "help" ? prefillHelp : undefined); onHome();
  ...
  function openWork(id: string) {
    ...
    clearEmbeddedParams(url);
    url.searchParams.set("view", selected?.productId === "tracker" ? "tracker" : "work");
    window.history.pushState(window.history.state, "", `${url.pathname}${url.search}${url.hash}`);
  }

  function openInquiry(tenantId: string) {
    onInquiry?.(tenantId);
    const url = new URL(window.location.href);
  ...
  const recent = <>
    ...
    {inquiryBusinesses.length > 0 && onInquiry && <section aria-label="Businesses"><h2>Businesses</h2>{inquiryBusinesses.slice(0, 8).map((business) => <button key={business.id} type="button" onClick={() => openInquiry(business.id)}><MessageSquareText size={16} aria-hidden="true" /><span>{business.title}</span></button>)}</section>}
  </>;
  ...
  return <StrelvaShell appBase={appBase} signOut={signOut}
    active={home ? section : "work"}
    title={!home ? inquiry ? "Inquiry work" : tracker !== undefined ? "Tracker" : agency ? "People & access" : selectedWork ? "Your work" : "New assessment" : section === "home" ? "Strelva" : section === "products" ? "Explore" : section === "help" ? "Help & service" : "My work"}
    ...
    actions={!home ? inquiry ? <button type="button" onClick={() => navigate("work")}>My work</button> : tracker !== undefined ? selectedWork ? <button type="button" onClick={onAgency}>Sharing & access</button> : <button type="button" onClick={() => navigate("work")}>My work</button> : <button type="button" onClick={agency ? () => navigate("home") : onAgency}>{agency ? "Back to home" : "Sharing & access"}</button> : undefined}
    notice={notice} contentId="workspace-main"
  >
    <div ref={scrollRef} className={styles.scroll} aria-busy={busy || undefined}>
      {!home ? <div className={styles.detail}><button type="button" className={styles.back} onClick={() => navigate("work")}><ArrowLeft size={16} />Back to my work</button>{inquiry ? <InquiryServerWorkspaceExperience tenantId={inquiry.tenantId} adapter={inquiry.adapter} initialSnapshot={inquiry.initialSnapshot} initialView={inquiry.initialView} initialRequestId={inquiry.initialRequestId} initialInquiryId={inquiry.initialInquiryId} basePath="/workspace" routePrefix="inquiry" /> : tracker !== undefined ? tracker : children}</div> : section === "help" ? <WorkspaceHelp key={helpRequest} workspaceName={current?.name} hasManagedService={sites.length > 0} onAgency={onAgency} initialRequest={helpRequest} /> : section === "products" ? <div className={styles.page}>
        {product ? <>
          ...
          <div className={styles.productList}>{products.map(item => <button type="button" key={item.id} className={styles.productRow} onClick={() => setProductId(item.id)}><span className={styles.productSymbol}>{item.id === "ai_visibility" ? <FileSearch size={26} strokeWidth={1.3} /> : <Globe2 size={26} strokeWidth={1.3} />}</span><span><strong>{item.name}</strong><p>{item.description}</p><small>{item.id === "homefinder" ? "Preview · early access" : item.availability === "available" ? "Available · Free assessment" : item.availability === "managed" ? "For connected clients" : "Not available yet"}</small></span><ArrowRight size={18} aria-hidden="true" /></button>)}</div>
        ...
        <div className={styles.starts}>
          ...
          {inquiryBusinesses.length > 0 && onInquiry ? <button type="button" onClick={() => openInquiry(inquiryBusinesses[0]!.id)}><MessageSquareText size={21} strokeWidth={1.5} /><span><strong>Handle customer inquiries</strong><small>Move one request forward</small></span><ArrowRight size={16} /></button> : null}
