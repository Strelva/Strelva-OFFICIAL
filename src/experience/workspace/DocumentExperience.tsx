...
import type { WorkspaceDocument } from "@/products/documents/engine";
...
type Props = { workspaceId: string; workId?: string; readOnly?: boolean; onSaved?: (workId: string) => void; transport?: DocumentTransport; initialRequestText?: string };

export function DocumentExperience(props: Props) {
  return <DocumentSession key={`${props.workspaceId}:${props.workId ?? "new"}`} {...props} />;
}

function DocumentSession({ workspaceId, workId, readOnly, onSaved, transport = serverTransport, initialRequestText }: Props) {
  ...
  async function save(undo = false) {
    ...
    try {
      ...
      setSaved(result); setTitle(result.document.title); setText(result.document.text); setReviewing(false);
  ...
  return <section className="mx-auto max-w-3xl space-y-6 p-4 sm:p-8" aria-label="Workspace document" aria-busy={busy}>
    ...
    {initialRequestText && !saved ? <aside className="space-y-2 border-y border-gray-border py-4 text-sm"><h2 className="font-medium">What you asked</h2><p className="whitespace-pre-wrap">{initialRequestText}</p><p className="text-gray-muted">Write or paste your document below. To have Strelva draft it, return to New and choose Prepare a plan.</p></aside> : null}
    ...
    {readOnly ? <article className="space-y-4"><p className="text-sm text-gray-muted">You have read-only access.</p><p className="whitespace-pre-wrap">{saved?.document.text}</p></article> : <form className="space-y-4" onSubmit={event => { event.preventDefault(); setReviewing(true); }}>
      <TextInput label="Document title" value={title} maxLength={160} required disabled={disabled} onChange={event => { setTitle(event.target.value); setReviewing(false); }} />
      <TextArea label="Document text" value={text} rows={14} maxLength={50000} disabled={disabled} onChange={event => { setText(event.target.value); setReviewing(false); }} />
      {saved && dirty ? <Button type="button" variant="secondary" disabled={disabled} onClick={() => { setTitle(saved.document.title); setText(saved.document.text); setReviewing(false); }}>Discard unsaved changes</Button> : null}
      {!reviewing ? <Button type="submit" disabled={disabled || !dirty || !title.trim()}>Review document</Button> : <section aria-label="Document change review" className="space-y-4 border-y border-gray-border py-4">
        <h2 className="font-display text-xl">Review before saving</h2>
        ...
        <Button type="button" disabled={disabled} onClick={() => void save()}>Save document</Button>
      </section>}
    </form>}
