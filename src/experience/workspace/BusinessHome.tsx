    ...
    <div className={styles.home} aria-busy={busy || undefined}>
      ...
      <section className={styles.section} aria-labelledby="home-attention">
        ...
        {busy || deliveryPending ? <p role="status" className={styles.muted}>Checking your work…</p> : attentionCount ? <ul className={styles.list}>
          ...
          {home.attention.map(({ work, reason }) => <li key={work.id}><button type="button" className={styles.row} aria-label={`Open ${work.title}`} onClick={() => onOpen(work.id)}><span><strong>{work.title}</strong><small>{reason}</small></span><ArrowRight size={16} aria-hidden="true" /></button></li>)}
