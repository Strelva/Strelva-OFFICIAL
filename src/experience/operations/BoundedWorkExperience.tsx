...
function Create({ productId, sources, disabled, create }: CreateProps) {
  ...
  return <form onSubmit={event => void submit(event)} className="space-y-5">
    ...
    {productId === "applications" ? <><p className="text-sm text-gray-muted">A private workspace app using fixed forms and record views. You will maintain it. It becomes available to workspace members after its checks pass.</p><fieldset className={group}><legend className="font-medium">Information to collect</legend>{fields.map((field, index) => <div key={field.id} className="grid gap-3 sm:grid-cols-[1fr_150px_auto]"><TextInput label={`Field ${index + 1}`} value={field.label} maxLength={80} required disabled={disabled} onChange={event => setFields(values => values.map((value, i) => i === index ? { ...value, label: event.target.value } : value))} /><label className="text-sm">Type<select className={control} value={field.type} disabled={disabled} onChange={event => setFields(values => values.map((value, i) => i === index ? { ...value, type: event.target.value as "text" | "number" | "boolean" } : value))}><option value="text">Text</option><option value="number">Number</option><option value="boolean">Yes or no</option></select></label><label className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" checked={field.required} disabled={disabled} onChange={event => setFields(values => values.map((value, i) => i === index ? { ...value, required: event.target.checked } : value))} />Required</label></div>)}<Button type="button" variant="secondary" disabled={disabled || fields.length >= 30} onClick={() => setFields(values => [...values, { id: `field_${values.length + 1}`, label: "", type: "text", required: false }])}>Add a field</Button></fieldset><fieldset className={group}><legend className="font-medium">What people can use</legend>{(["form", "list", "detail", "document"] as const).map(kind => <label key={kind} className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={kinds.includes(kind)} disabled={disabled} onChange={event => setKinds(values => event.target.checked ? [...values, kind] : values.filter(value => value !== kind))} />{kind === "form" ? "Enter a record" : kind === "list" ? "Browse records" : kind === "detail" ? "Inspect a record" : "Read a record as a document"}</label>)}</fieldset></> : productId === "scheduling" ? <><p className="text-sm text-gray-muted">Reserve time inside this workspace. This schedule is not connected to an external calendar. Times use your device’s timezone.</p><TimeFields start={start} end={end} setStart={setStart} setEnd={setEnd} disabled={disabled} /></> : <><p className="text-sm text-gray-muted">Compare two saved sources in this workspace. Each check records the exact source versions. Automatic checks require background execution to be enabled. You can run a due check here.</p><SourcePicker label="First source" value={left} onChange={setLeft} sources={sources} excludedId={right.workId} disabled={disabled} /><SourcePicker label="Second source" value={right} onChange={setRight} sources={sources} excludedId={left.workId} disabled={disabled} /><TextInput label="Minimum minutes between checks" type="number" min={15} max={43200} value={interval} onChange={event => setInterval(event.target.value)} required disabled={disabled} /></>}
...
function TimeFields({ start, end, setStart, setEnd, disabled }: { start: string; end: string; setStart: (value: string) => void; setEnd: (value: string) => void; disabled: boolean }) {
  return <div className="grid gap-4 sm:grid-cols-2"><TextInput label="Starts" type="datetime-local" value={start} onChange={event => setStart(event.target.value)} required disabled={disabled} /><TextInput label="Ends" type="datetime-local" value={end} onChange={event => setEnd(event.target.value)} required disabled={disabled} /></div>;
}
  ...
  return <>
    ...
    <section className={`${group} space-y-8`} aria-labelledby={`${id}-live-app-heading`}>
      ...
      <div className="space-y-8" aria-label="Private application">{components.map((component, index) => {
        ...
        if (component.kind === "form") return <details key={index} open={!value.records.length} className={group}><summary className="cursor-pointer text-sm">{value.records.length ? "Add another record" : "Add a first record"}</summary><form onSubmit={event => void submit(event, component.fields)} className={group}><h2 className="font-display text-xl">Add a record</h2>{fields.map(field => field.type === "boolean" ? <label key={field.id} className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={values[field.id] === true} disabled={disabled || !canUse || value.recordsRevision === undefined} onChange={event => setValues(current => ({ ...current, [field.id]: event.target.checked }))} />{field.label}</label> : <TextInput key={field.id} label={field.label} type={field.type === "number" ? "number" : "text"} step={field.type === "number" ? "any" : undefined} value={values[field.id] === undefined ? "" : String(values[field.id])} maxLength={field.type === "text" ? 10000 : undefined} required={field.required} disabled={disabled || !canUse || value.recordsRevision === undefined} onChange={event => setValues(current => ({ ...current, [field.id]: field.type === "number" && event.target.value !== "" ? Number(event.target.value) : event.target.value }))} />)}<Button type="submit" disabled={disabled || !canUse || value.recordsRevision === undefined}>Save record</Button></form></details>;
...
function ScheduleResult({ value, command, disabled }: { value: Schedule; command: Command; disabled: boolean }) {
  ...
  return <><p className="text-sm text-gray-muted">Reservations are held in this workspace. No external calendar is connected. Times use your device’s timezone.</p><section className={group}><h2 className="font-display text-xl">Available windows</h2><ul className="space-y-2 text-sm">{value.availability.map((slot, index) => <li key={index}>{time(slot.start)} to {time(slot.end)}</li>)}</ul></section><section className={group}><h2 className="font-display text-xl">Reservations</h2>{value.reservations.length ? <ul className="space-y-4">{value.reservations.map(reservation => <li key={reservation.requestId} className="space-y-2 text-sm"><strong>{reservation.title}</strong><p>{time(reservation.start)} to {time(reservation.end)}</p><p>{reservation.status === "reserved" ? "Reserved in workspace" : reservation.status === "cancelled" ? "Cancelled" : reservation.status === "accepted" ? `Provider accepted · verification ${reservation.verification || "pending"}` : "Provider confirmation unresolved; do not repeat the reservation."}</p>{reservation.status === "reserved" ? <Button variant="secondary" disabled={disabled} onClick={() => void command({ kind: "cancel", requestId: reservation.requestId })}>Cancel {reservation.title}</Button> : null}</li>)}</ul> : <p className="text-sm text-gray-muted">No time has been reserved yet.</p>}</section><form className={group} onSubmit={event => void reserve(event)}><h2 className="font-display text-xl">Reserve a time</h2><TextInput label="Reservation name" value={title} required maxLength={160} disabled={disabled} onChange={event => setTitle(event.target.value)} /><TimeFields start={start} end={end} setStart={setStart} setEnd={setEnd} disabled={disabled} />{error ? <p role="alert" className="text-sm text-critical">{error}</p> : null}<Button type="submit" disabled={disabled || !title.trim()}>Reserve in workspace</Button></form></>;
}
...
function EditApplicationSpec({ value, command, disabled }: { value: Application; command: Command; disabled: boolean }) {
  ...
  function updateField(fieldId: string, update: Partial<Spec["fields"][number]>) {
    setEditorError("");
