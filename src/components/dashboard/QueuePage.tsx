 ...
 * live. New content, so it renders as an added block (the diff's "added" styling).
 */
function readGbpPost(event: UnifiedEvent): { summary: string; ctaUrl?: string } | null {
  ...
  const ctaUrl = typeof event.metadata?.ctaUrl === "string" && event.metadata.ctaUrl.trim()
 ...
 */
function readGbpHours(event: UnifiedEvent): GbpHour[] | null {
...
/**
 * The read-only detail under a pending card: the GBP post text / proposed hours
 * for a Google draft, else the field-level before→after diff for a content edit.
 * So an approver always sees the actual thing they're approving, not just a label.
 */
function QueueEventDetail({ event }: { event: UnifiedEvent }) {
  const post = readGbpPost(event);
  if (post) return <QueueGbpPost post={post} />;
  const hours = readGbpHours(event);
  if (hours) return <QueueGbpHours hours={hours} />;
  return <QueueDiff diffs={readDiffs(event)} />;
}
