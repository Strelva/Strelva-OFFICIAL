...
export const serviceRequestSchema = z.object({
  ...
  request: z.string().min(1).max(3_000),
  outcome: z.string().min(1).max(3_000),
  context: z.record(z.string(), z.unknown()),
  scope: z.array(z.string().min(1).max(120)).min(1).max(16),
  provider: serviceRequestProviderSchema,
  providerAcceptance: serviceRequestProviderAcceptanceSchema,
...
export class ServiceRequestValidationError extends Error {
  constructor(message = "The service request is invalid.") {
    super(message);
...
export class ServiceRequestAccessError extends Error {
  constructor(message = "This service request is unavailable to your account.") {
    super(message);
    this.name = "ServiceRequestAccessError";
  }
}

export class ServiceRequestConflictError extends Error {
  constructor(message = "The service request changed. Reload before continuing.") {
    super(message);
...
export class ServiceRequestNotFoundError extends Error {
  constructor(message = "The service request was not found.") {
    super(message);
...
export class ServiceRequestStoreError extends Error {
  constructor(message = "Service request storage is unavailable.") {
    super(message);
    this.name = "ServiceRequestStoreError";
  }
}
