...
describe("business setup HTTP boundary", () => {
  ...
  it.each([{origin:"https://outside.example"},{"sec-fetch-site":"cross-site"}])("denies cross-site mutations before actor resolution",async headers=>{expect((await POST(request("{}",headers))).status).toBe(403);expect(mocks.actor).not.toHaveBeenCalled();});
