...
export async function recordApproval(tenantId: string): Promise<number> {
  ...
  try {
    ...
    // EXPIRE NX: set expiry only if the key has no TTL (not already set).
    // Non-atomic relative to INCR, but acceptable since auto-approval is a
    // convenience optimization, not a security boundary.
    await redis.expire(key, 90 * 24 * 60 * 60, "NX");
    return newCount;
  } catch {
    return 0;
  }
