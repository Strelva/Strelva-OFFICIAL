...
export async function sendDomainAlertEmail(params: {
  ...
  try {
    ...
    for (const d of down) {
      rows.push({ label: `DOWN · ${cleanSubjectText(d.siteName)}`, value: `${d.host} — ${cleanSubjectText(d.problem)}` });
    }
    for (const e of expiring) {
      rows.push({ label: `Expiring · ${cleanSubjectText(e.siteName)}`, value: `${e.host} — ${cleanSubjectText(e.problem)}` });
    }
